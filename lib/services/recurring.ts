...
export async function createRecurringRule(
  ...
  // Validate the RRULE by computing the first occurrence from just before start.
  const first = nextOccurrence(input.rrule, new Date(start.getTime() - 1000));
  if (!first) return err("invalid", "RRULE produced no occurrences");
  const [row] = await db
    .insert(recurringRules)
    .values({ rrule: input.rrule, taskTemplate: input.taskTemplate, nextRunAt: first })
    .returning();
  return ok(row!);
}
